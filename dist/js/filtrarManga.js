const containerManga = document.querySelector(".container-manga");
const completoBtn = document.getElementById("completoBtn");
const comprandoBtn = document.getElementById("comprandoBtn");
const largadoBtn = document.getElementById("largadoBtn");
const pausadoBtn = document.getElementById("pausadoBtn");
...
async function buscarMangasPorStatus(status) {
    try {
        const busca = await fetch("https://sheet.best/api/sheets/52e3d802-5a8f-4172-9d29-b191990200a0");
        const mangas = await busca.json();
        if (!Array.isArray(mangas)) {
...
function filtrarPesquisa() {
    ...
    mangas.forEach((manga) => {
        ...
        if (mangaNome && mangaNome.includes(valorFiltro)) {
            manga.style.display = "block"; // Converter manga para HTMLElement
        }
        else {
            manga.style.display = "none"; // Converter manga para HTMLElement
        }
    });
}
buscarTodosMangas();
